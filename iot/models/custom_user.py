...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    def line_user(self,msg='Test message'):
        """Send an line to this user."""
        ACCESS_TOKEN = self.line_token
        headers = {"Authorization": f"Bearer {ACCESS_TOKEN}"}
        data = {"message": msg}
        requests.post("https://notify-api.line.me/api/notify",headers=headers,data=data,)
    
    def slack_user(self,msg='Test message'):
        """Send an slack to this user."""
        ACCESS_TOKEN = self.slack_token
        ACCESS_CHANNEL = self.slack_channel
        client = WebClient(token=ACCESS_TOKEN)
        client.chat_postMessage(channel=ACCESS_CHANNEL, text=msg)
